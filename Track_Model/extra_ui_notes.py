...
class Window(QMainWindow):
    def __init__(self, track_model):
        ...
        self.str_list_blocks = list(self.track_model.data[1:, 2].astype(str))
        self.combo1 = QComboBox()
        self.combo1.addItems(self.str_list_blocks)
        self.combo1.activated.connect(self.combo1_new_item_selected)
        ...
        self.failure_modes_group.setLayout(fm_group_layout)
        self.layout.addWidget(self.failure_modes_group, 0, 0)
        ...
        self.temperature_controls_group.setLayout(tc_layout)
        self.layout.addWidget(self.temperature_controls_group, 1, 0)
        ...
        self.upload_layout_group.setLayout(ul_layout)
        self.layout.addWidget(self.upload_layout_group, 4, 0, 1, 1)
        ...
        map_layout.addWidget(self.map_table)
        self.map_group.setLayout(map_layout)
        self.layout.addWidget(self.map_group, 0, 1)

        # Test Bench Button
        self.test_bench_button = QPushButton("Test Bench")
        self.test_bench_button.clicked.connect(self.test_bench_button_clicked)
        self.layout.addWidget(self.test_bench_button, 3, 0)
    ...
    def refresh(self):
        ...
        for i in range(1, m):
            for j in range(0, n):
                self.map_table.setItem(i - 1, j, QTableWidgetItem(str(self.track_model.get_data()[i, j])))
        self.ss_group_layout.addWidget(self.map_table)

        map_layout.addWidget(self.map_table)
        self.map_group.setLayout(map_layout)
        self.layout.addWidget(self.map_group, 1, 0)

        # failure drop-downs
        self.str_list_blocks = list(self.track_model.data[7, 1:].astype(str))
        self.combo1.clear()
        self.combo2.clear()
        self.combo3.clear()
        self.combo1.addItems(self.str_list_blocks)
