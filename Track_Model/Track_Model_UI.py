from Track_Model_Testbench import TestBench_JEB382
...
class SW_UI_JEB382(QMainWindow):
    ...
    def toggle_TB(self):
        if self.TB_window.isVisible():
            self.TB_window.hide()
            self.BTN_TB.setStyleSheet(
                "background-color: rgb(156, 156, 156); border: 2px solid rgb(100, 100, 100); border-radius: 6px")
            print("JEB382 TB Hidden")
        else:
            self.BTN_TB.setStyleSheet(
                "background-color: rgb(143, 186, 255); border: 2px solid rgb(42, 97, 184); border-radius: 6px")
            self.TB_window.show()
    ...
    def genSect2_2(self, OriginY, OriginX):
        ...
        label_names = ["Actual Speed", "Commanded Speed", "Cabin Lights", "Headlights", "Cabin Temp",
                       "Doors (Left, Right)"]
        for i in range(1, len(label_names) + 1):
            label = better_label_ret(label_names[i - 1])
