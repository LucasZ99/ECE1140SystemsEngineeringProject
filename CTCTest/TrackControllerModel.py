...
class TrackControllerModel(QObject):
    def __init__(self):
        ...
        CTCTestSignals.update_wayside_from_ctc.connect(self.update_wayside_from_ctc)
    ...
    def update_track_signals(self, track_signals: list[TrackSignal]) -> None:
        ...
        CTCTestSignals.ui_update_track_signals.emit(track_signals)
    ...
    def update_wayside_from_ctc(self, track_signals: list[TrackSignal],
        ...
        if track_signals.__len__() > 0:
            self.update_track_signals(track_signals)
            track_signals.clear()
    ...
    def wayside_get_track_signals_signal_handler(self):
        CTCTestSignals.ui_update_track_signals.emit(self.track_signals)

    def wayside_get_occupancies_signal_handler(self):
        for block in self.block_occupancies:
            CTCTestSignals.ui_update_block_occupancy_signal.emit(block, self.block_occupancies[block])

    def connect_to_frontend_signals(self):
        CTCTestSignals.wayside_update_block_occupancy.connect(self.update_block_occupancy)
        CTCTestSignals.wayside_get_track_signals_signal.connect(self.wayside_get_track_signals_signal_handler)
        CTCTestSignals.wayside_get_occupancies_signal.connect(self.wayside_get_occupancies_signal_handler)
